...
\texttt{User} holds associations with \texttt{Role} (GUEST, READER, LIBRARIAN, ADMIN) and \texttt{UserStatus} (PENDING\_VERIFICATION, ACTIVE, SUSPENDED, BANNED). \texttt{User} associates $1 - 0..*$ with \texttt{OtpToken} (method \texttt{isValid()}) and $1 - 0..*$ with \texttt{BorrowRecord}.
